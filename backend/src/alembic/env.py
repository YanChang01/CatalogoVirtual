...
def run_migrations_sync(connection: AsyncConnection, target_metadata: any):
    #Run migrations en contexto asíncrono.
...
#Lógica de ejecución de migraciones.
if context.is_offline_mode():
    run_migrations_offline()
else:
    try:
        #Obtener el loop que está corriendo.
        loop = get_running_loop()
    except RuntimeError:
        #Crear un nuevo loop y establecerlo en la configuración.
        loop = new_event_loop()
        set_event_loop(loop)
    
    #Correr el nuevo loop establecido.
